 ...
 */
u8              Bot_State;
u8              Bulk_Data_Buff[BULK_MAX_PACKET_SIZE]; /*
...
void            Mass_Storage_In(void)
{
   ...
   case BOT_ERROR:
      ...
      SetEPRxStatus(ENDP2, EP_RX_VALID); /*
                                          * enable the Endpoint to recive the 
                                          * next cmd
                                          */
      break;
   case BOT_DATA_IN:
...
void            CBW_Decode(void)
{
   ...
   if (Data_Len != BOT_CBW_PACKET_LENGTH)
   {
      Bot_Abort(BOTH_DIR);
      /*
       * reset the CBW.dSignature to desible the clear feature until
       * receiving a Mass storage reset
       */
      CBW.dSignature = 0;
