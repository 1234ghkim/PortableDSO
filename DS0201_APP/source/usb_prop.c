...
//  PowerOn();
   ...
   /*
    * Wait until device is configured  while
    * (pInformation->Current_Configuration == 0) { NOP_Process(); }
    */

   for (tmp = 0; tmp < 100000; ++tmp)
      NOP_Process(); // CXG:加上这段取代上面的死循环 12/08
...
RESULT          MASS_Get_Interface_Setting(u8 Interface, u8 AlternateSetting)
{
   ...
   {
      return USB_UNSUPPORT; /*
                             * in this application we don't have
                             * AlternateSetting
                             */
   } else if (Interface > 0)
   {
      return USB_UNSUPPORT; /*
                             * in this application we have only 1 interfaces
                             */
   }
   return USB_SUCCESS;
}
